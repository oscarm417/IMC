...
class Trader:
    ...
    def calculate_bid_ask_dynamic(self,smart_price,bid_price_1,ask_price_1,percent_of_max_bid = .8):
        ...
        smart_bid = smart_price - spread 
        smart_ask = smart_price +spread 

        return m.ceil(smart_bid), m.floor(smart_ask)
    ...
    def calculate_reservation_price(self, product: str, current_inventory: int, target_inventory: int, smart_price: float, gamma: float) -> float:
        """
        ...
        reservation_price = smart_price - ((current_inventory - target_inventory) * gamma * (abs_volatility)**2)
    ...
    def trade_logic(self, product:str, state: TradingState, market_variables: list):
        ...
        if lob_buy_volume_total > 0 and lob_sell_volume_total > 0:
            ...
            smart_price_bid, smart_price_ask = self.calculate_bid_ask_dynamic(smart_price,best_bid,best_ask,self.percent_of_max_bid)
            ...
            inventory_adj_bid, inventory_adj_ask = self.calculate_bid_ask_dynamic(smart_price,best_bid,best_ask,self.percent_of_max_bid )
